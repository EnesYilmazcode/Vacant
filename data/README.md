...
| --- | --- | --- | --- |
| `rooms-1268.json` | Ohio State public class schedule, `https://content.osu.edu/v2/classes/search` | Facts. No rights asserted. Not MIT: that is a software licence and does not fit a dataset | Built 2026-08-27 by `scripts/build-index.mjs` from a walk-until-stable harvest of term 1268. 545 requests over 4 passes. 871 rooms, 96 buildings |
| `buildings-1268.json` | Same GIS layer as `buildings.json`, sliced to the 96 buildings that host a class this term | (c) 2025 The Ohio State University, Facilities Information and Technology Services | Split out of `buildings.json` by `scripts/build-index.mjs`. 7.4 KB against the full table's 167 KB |
...
| `campus.json` | `https://gissvc.osu.edu/arcgis/rest/services/Data/FacilitiesStreets_RO/MapServer`, layers 11, 12, 9 and 13 | (c) 2025 The Ohio State University, Facilities Information and Technology Services | Pulled 2026-08-27 by `scripts/fetch-campus.mjs`. `where=1=1` intersected with a bounding box anchored on class-hosting buildings within 2 km of the Oval. 1,544 features, 13,258 points, 39 KB gzipped |
