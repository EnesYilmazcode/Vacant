...
**ANSWERED 2026-09-02, in commit `882471a`, see [#89](https://github.com/EnesYilmazcode/Vacant/issues/89).** Built as recommended, ranked down rather than hidden. `ga` is a term in `tierOf` (`js/engine.js`), so it is settled before walk time the way published hours already are, and it sits inside the wait and the window: a departmental room that is free when you arrive still beats a general-assignment one you would wait an hour for. The one word on the row is "departmental", on the list row and on the room screen. Re-derived on the committed index, which is 425 rooms rather than the 871 this was written against: 98 rooms are absent from the general-assignment list, 56 of them types the first rung offers by default, and the 77 type-1B figure below is now 41. Replayed from the Oval over 250 lists, row one was a departmental room in 32 of them (12.8%) before and 0 (0.0%) after, departmental rows in the top ten fell from 30.8% to 0.3%, and departmental rooms are still 15.4% of the rows shown. Nothing was deleted from the index: filtering them out of the first rung stays unbuilt, and the ground-truth walk ([#26](https://github.com/EnesYilmazcode/Vacant/issues/26)) is what should decide whether it is ever needed.
...
- **Union room identities across three terms so zero-class rooms are discoverable. DROPPED 2026-08-27, measured, not deferred.** [#30](https://github.com/EnesYilmazcode/Vacant/issues/30) ran the funnel over all three committed terms and found **95 carried-forward rooms and zero strong positives**: not one is both a type Vacant would show and on the Registrar's general assignment list. All 327 general assignment rooms appear in all three terms, so there is no such thing as an unscheduled one. 33 of the 49 weak positives are Knowlton Hall architecture studio bays. The 190-zero-class-rooms figure came from a competitor's CDN and reads as cross-term residue in their own index. Full measurement in [`docs/research/spike-unscheduled-rooms.md`](research/spike-unscheduled-rooms.md).
