...
function timeline({ known, blocks, open, close, now }) {
  const rows = [];
  let cursor = open;
  ...
  return { known, rows, blocks, open, close, now };
}
...
test('a class in the room is a claim either way, hours or no hours', () => {
  for (const known of [true, false]) {
    const c = roomClaim(timeline({
      ...
      open: known ? at(7) : at(12, 45), close: known ? at(22) : at(16), now: at(13),
    }));
    assert.equal(c.kind, 'in-class');
    assert.equal(c.until, at(14, 5));
    assert.equal(c.next, at(14, 5));
    assert.equal(c.nextLen, 55);
  }
});
